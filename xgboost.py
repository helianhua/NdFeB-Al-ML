...
BO_PARAM_BOUNDS = {
    'n_estimators': (50, 500), 'learning_rate': (0.01, 0.3), 'max_depth': (3, 10),
    'subsample': (0.6, 1.0), 'colsample_bytree': (0.6, 1.0), 'gamma': (0, 5)
}

# === Helper Functions ===
def bo_evaluate(X_train, y_train, X_val, y_val, n_estimators, learning_rate, max_depth, subsample, colsample_bytree, gamma):
    params = {
...
def estimate_uncertainty_shap(model, X):
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(X)
    return np.std(shap_values, axis=1)

def select_samples_with_budget(X_pool, X_train, uncertainty, budget_per_round, lambda_rep=0.7):
    ...
    num_samples_to_select = min(budget_per_round, len(scores))
...
def run_learning_process(strategy, X_train_base, y_train_base, X_pool_base, y_pool_base, X_val, y_val, X_test, y_test):
    ...
    test_metrics_list, train_size_list, all_captured_states = [], [], {}

    for round_i in range(ACTIVE_ROUNDS):
        train_size_list.append(len(X_train))
        def bo_target_func(**params): return bo_evaluate(X_train, y_train, X_val, y_val, **params)
        optimizer = BayesianOptimization(f=bo_target_func, pbounds=BO_PARAM_BOUNDS, random_state=round_i*10, verbose=0)
        optimizer.maximize(init_points=3, n_iter=5)
        best_params_round = optimizer.max['params']
        model_params = {'n_estimators': int(best_params_round['n_estimators']), 'learning_rate': best_params_round['learning_rate'],
                        'max_depth': int(best_params_round['max_depth']), 'subsample': best_params_round['subsample'],
                        'colsample_bytree': best_params_round['colsample_bytree'], 'gamma': best_params_round['gamma'],
                        'random_state': 42, 'verbosity': 0, 'n_jobs': -1}
        ...
        if strategy == 'Active Learning':
            selected_idx = select_samples_with_budget(X_pool, X_train, estimate_uncertainty_shap(model, X_pool), BUDGET_PER_ROUND, LAMBDA_REP)
        else:
...
# === Main Workflow: Run Robustness Experiment ===
def main_robustness_workflow(data_path, results_save_path="model_output/br_xgb_robustness_results.joblib"):
    ...
    for i, seed in enumerate(RANDOM_SEEDS):
        ...
        full_model_params = {'n_estimators': int(best_full_params['n_estimators']), 'learning_rate': best_full_params['learning_rate'],
                             'max_depth': int(best_full_params['max_depth']), 'subsample': best_full_params['subsample'],
                             'colsample_bytree': best_full_params['colsample_bytree'], 'gamma': best_full_params['gamma'],
                             'random_state': 42, 'verbosity': 0, 'n_jobs': -1}
        full_model = XGBRegressor(**full_model_params)
