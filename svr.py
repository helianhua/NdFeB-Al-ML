...
def bo_evaluate(X_train, y_train, X_val, y_val, C, epsilon):
    model = BaggingRegressor(estimator=SVR(C=C, epsilon=epsilon), n_estimators=10, random_state=42, n_jobs=-1)
...
def run_learning_process(strategy, X_train_base, y_train_base, X_pool_base, y_pool_base, X_val, y_val, X_test, y_test):
    ...
    for round_i in range(ACTIVE_ROUNDS):
        ...
        def bo_target_func(C, epsilon):
            return bo_evaluate(X_train, y_train, X_val, y_val, C, epsilon)
        ...
        model = BaggingRegressor(
            estimator=SVR(C=best_params['C'], epsilon=best_params['epsilon']),
...
def main_robustness_workflow(data_path, results_save_path="model_output/br_svr_robustness_results.joblib"):
    ...
    for i, seed in enumerate(RANDOM_SEEDS):
        ...
        def bo_full_model(C, epsilon):
            X_bo_train, _, y_bo_train, _ = train_test_split(X_temp, y_temp, test_size=0.1, random_state=seed)
            return bo_evaluate(X_bo_train, y_bo_train, X_val, y_val, C, epsilon)
        ...
        print(f"Best params for Full Dataset: C={best_full_params['C']:.4f}, epsilon={best_full_params['epsilon']:.4f}")

        full_model = BaggingRegressor(
            estimator=SVR(C=best_full_params['C'], epsilon=best_full_params['epsilon']),
            n_estimators=10, random_state=42, n_jobs=-1
        )
